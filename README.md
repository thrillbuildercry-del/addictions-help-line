...
## Features
- Firebase Auth with email/password sign-in + self-service sign-up
- Firestore profile role (`admin` / `worker`)
- Role-protected frontend routes and Firestore rules
- Modern mobile-first UI (fast entry flow + bottom navigation)
...
Sign up from `/login` to gain access. Newly signed-up users are created as `worker` by default. Admins can still manage roles from the admin user management flow.
